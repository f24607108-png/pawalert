...
class Report(db.Model):
    ...
    # User-confirmed values (override AI if provided)
    user_species        = db.Column(db.String(50), nullable=True)
    user_severity       = db.Column(db.String(20), nullable=True)
    ...
    def to_dict(self):
        return {
            ...
            "predicted_species": self.user_species or self.predicted_species,
            "species_confidence": round(self.species_confidence or 0, 2),
            "predicted_severity": self.user_severity or self.predicted_severity,
            "severity_confidence": round(self.severity_confidence or 0, 2),
            "user_species": self.user_species,
            "user_severity": self.user_severity,
            "ai_species": self.predicted_species,
            "ai_severity": self.predicted_severity,
...
/* ── SPECIES & SEVERITY SELECTORS ── */
.species-btn{background:rgba(255,255,255,.05);border:1.5px solid rgba(255,255,255,.1);border-radius:12px;padding:.7rem .4rem;text-align:center;cursor:pointer;transition:all .2s;}
.species-btn:hover{background:rgba(0,230,118,.08);border-color:rgba(0,230,118,.3);}
.species-btn.selected{background:rgba(0,230,118,.15);border-color:#00e676;box-shadow:0 0 12px rgba(0,230,118,.2);}
.sp-icon{font-size:1.6rem;line-height:1;margin-bottom:.25rem;}
.sp-label{font-size:.75rem;font-weight:600;color:rgba(255,255,255,.7);}
.species-btn.selected .sp-label{color:#00e676;}
.severity-btn{border-radius:12px;padding:.75rem .4rem;text-align:center;cursor:pointer;transition:all .2s;border:1.5px solid;}
.mild-btn{background:rgba(0,230,118,.07);border-color:rgba(0,230,118,.2);color:rgba(255,255,255,.7);}
.mild-btn:hover,.mild-btn.selected{background:rgba(0,230,118,.18);border-color:#00e676;color:#fff;}
.moderate-btn{background:rgba(255,171,64,.07);border-color:rgba(255,171,64,.2);color:rgba(255,255,255,.7);}
.moderate-btn:hover,.moderate-btn.selected{background:rgba(255,171,64,.18);border-color:#ffab40;color:#fff;}
.critical-btn{background:rgba(255,82,82,.07);border-color:rgba(255,82,82,.2);color:rgba(255,255,255,.7);}
.critical-btn:hover,.critical-btn.selected{background:rgba(255,82,82,.18);border-color:#ff5252;color:#fff;}

/* ── RECORDS TABLE ── */
.records-wrap{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:var(--radius);overflow:hidden;}
.records-wrap table{width:100%;border-collapse:collapse;}
.records-wrap th{background:rgba(255,255,255,.05);color:var(--text-muted);font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;padding:.6rem 1rem;font-weight:600;text-align:left;}
.records-wrap td{padding:.65rem 1rem;border-top:1px solid rgba(255,255,255,.04);font-size:.82rem;color:var(--text);vertical-align:middle;}
.records-wrap tr:hover td{background:rgba(0,230,118,.03);}
.records-wrap tr{cursor:pointer;}
  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="nav">
      <ul class="navbar-nav ms-auto align-items-center gap-1">
        ...
        <li class="nav-item"><a class="nav-link" href="/records"><i class="fa fa-database me-1"></i>Records</a></li>
...
def home():
    if request.method == "POST":
        ...
        manual_species  = request.form.get("manual_species", "").strip()
        manual_severity = request.form.get("manual_severity", "").strip()
        ...
        try:
            ...
            ai_species, sp_conf  = result["species"], result["species_confidence"]
            ai_severity, sv_conf = result["severity"], result["severity_confidence"]
        except:
            ai_species = ai_severity = "unknown"; sp_conf = sv_conf = 0.0
        # User selection overrides AI — use AI as fallback only
        final_species  = manual_species  if manual_species  in SPECIES_CLASSES  else ai_species
        final_severity = manual_severity if manual_severity in SEVERITY_CLASSES else ai_severity
        report = Report(
            user_id=current_user.id if current_user.is_authenticated else None,
            latitude=lat, longitude=lon, address_text=addr, city=city,
            image_filename=fname, description=desc,
            predicted_species=ai_species, species_confidence=sp_conf,
            predicted_severity=ai_severity, severity_confidence=sv_conf,
            user_species=manual_species if manual_species in SPECIES_CLASSES else None,
            user_severity=manual_severity if manual_severity in SEVERITY_CLASSES else None,
  ...
  <div class="col-lg-7">
    <div class="glass-card p-4">
      ...
      <form method="POST" enctype="multipart/form-data" id="reportForm">
        ...
        <div class="mb-3">
          <label class="form-label">🐾 What animal is it? <span style="color:#ff5252;">*</span></label>
          <div class="row g-2" id="speciesGrid">
            <div class="col-4"><div class="species-btn" onclick="selectSpecies('dog',this)"><div class="sp-icon">🐶</div><div class="sp-label">Dog</div></div></div>
            <div class="col-4"><div class="species-btn" onclick="selectSpecies('cat',this)"><div class="sp-icon">🐱</div><div class="sp-label">Cat</div></div></div>
            <div class="col-4"><div class="species-btn" onclick="selectSpecies('cow',this)"><div class="sp-icon">🐄</div><div class="sp-label">Cow</div></div></div>
            <div class="col-4"><div class="species-btn" onclick="selectSpecies('donkey',this)"><div class="sp-icon">🫏</div><div class="sp-label">Donkey</div></div></div>
            <div class="col-4"><div class="species-btn" onclick="selectSpecies('bird',this)"><div class="sp-icon">🐦</div><div class="sp-label">Bird</div></div></div>
            <div class="col-4"><div class="species-btn" onclick="selectSpecies('other',this)"><div class="sp-icon">🐾</div><div class="sp-label">Other</div></div></div>
          </div>
          <input type="hidden" name="manual_species" id="manualSpecies">
          <div id="speciesHint" style="font-size:.75rem;color:rgba(0,230,118,.6);margin-top:.4rem;display:none;">✓ Species selected — AI will also analyse your photo</div>
        </div>

        <div class="mb-4">
          <label class="form-label">🩺 How bad does it look? <span style="color:#ff5252;">*</span></label>
          <div class="row g-2">
            <div class="col-4"><div class="severity-btn mild-btn" onclick="selectSeverity('mild',this)"><div style="font-size:1.3rem;margin-bottom:.2rem;">🟢</div><div style="font-size:.82rem;font-weight:600;">Mild</div><div style="font-size:.68rem;opacity:.7;">Walking, minor injury</div></div></div>
            <div class="col-4"><div class="severity-btn moderate-btn" onclick="selectSeverity('moderate',this)"><div style="font-size:1.3rem;margin-bottom:.2rem;">⚠️</div><div style="font-size:.82rem;font-weight:600;">Moderate</div><div style="font-size:.68rem;opacity:.7;">Wound, limping</div></div></div>
            <div class="col-4"><div class="severity-btn critical-btn" onclick="selectSeverity('critical',this)"><div style="font-size:1.3rem;margin-bottom:.2rem;">🔴</div><div style="font-size:.82rem;font-weight:600;">Critical</div><div style="font-size:.68rem;opacity:.7;">Collapsed, severe</div></div></div>
          </div>
          <input type="hidden" name="manual_severity" id="manualSeverity">
          <div id="severityHint" style="font-size:.75rem;color:rgba(0,230,118,.6);margin-top:.4rem;display:none;">✓ Severity selected</div>
        </div>
        <div class="mb-3">
        ...
        <div class="row g-3 mb-4">
          <div class="col-md-6">
            <label class="form-label">👤 Your Name <span style="color:rgba(255,255,255,.3);font-size:.7rem;">(optional)</span></label>
            <input type="text" name="reporter_name" class="form-control" placeholder="So NGO can contact you">
          </div>
          <div class="col-md-6">
            <label class="form-label">📞 Your Phone <span style="color:rgba(255,255,255,.3);font-size:.7rem;">(optional)</span></label>
            <input type="tel" name="reporter_phone" class="form-control" placeholder="+92 300...">
          </div>
        </div>
...
document.getElementById("reportForm").addEventListener("submit",function(e){
  ...
  if(!document.getElementById("manualSpecies").value){e.preventDefault();showToast('error','Species Required','Please select the type of animal.');return;}
  if(!document.getElementById("manualSeverity").value){e.preventDefault();showToast('error','Severity Required','Please select how bad the animal looks.');return;}
...
function selectSpecies(val, el){
  document.querySelectorAll('.species-btn').forEach(b=>b.classList.remove('selected'));
  el.classList.add('selected');
  document.getElementById('manualSpecies').value=val;
  document.getElementById('speciesHint').style.display='block';
  showToast('info','Species Selected',el.querySelector('.sp-label').textContent+' selected. AI will also analyse your photo.');
}
function selectSeverity(val, el){
  document.querySelectorAll('.severity-btn').forEach(b=>b.classList.remove('selected'));
  el.classList.add('selected');
  document.getElementById('manualSeverity').value=val;
  document.getElementById('severityHint').style.display='block';
}
</script>"""
...
def records():
    ...
    if species:  q = q.filter(Report.predicted_species == species)
    if severity: q = q.filter(Report.predicted_severity == severity)
    if status:   q = q.filter(Report.status == status)
    if city:     q = q.filter(Report.city.ilike(f"%{city}%"))
    total_count = q.count()
    reports = q.limit(50).offset((page-1)*50).all()
    sev_cls = {"mild":"sev-mild","moderate":"sev-moderate","critical":"sev-critical"}
    sp_em   = {"cat":"🐱","dog":"🐶","cow":"🐄","bird":"🐦","donkey":"🫏","other":"🐾"}
    stcol   = {"pending":"#888","dispatched":"#ffab40","in_progress":"#40c4ff","rescued":"#00e676","closed":"#666"}

    rows = ""
    for r in reports:
        final_sp  = r.user_species  or r.predicted_species  or "unknown"
        final_sev = r.user_severity or r.predicted_severity or "unknown"
        confirmed = " ✓" if r.user_species else " (AI)"
        reporter  = r.reporter_name or (r.reporter.name if r.reporter else "Anonymous")
        rows += f"""<tr onclick="window.location='/track/{r.id}'">
          <td><span style="color:#00e676;font-weight:600;">#{r.id}</span>{' <span class="sev-badge sos-badge" style="font-size:.65rem;padding:.15rem .5rem;">SOS</span>' if r.is_sos else ''}</td>
          <td>{sp_em.get(final_sp,"🐾")} {final_sp.title()}<span style="font-size:.68rem;color:rgba(0,230,118,.5);">{confirmed}</span></td>
          <td><span class="sev-badge {sev_cls.get(final_sev,'sev-unknown')}">{final_sev}</span></td>
          <td style="color:{stcol.get(r.status,'#888')};font-weight:600;font-size:.78rem;">{r.status.replace('_',' ').title()}</td>
          <td style="color:var(--text-muted);font-size:.78rem;">{r.city or r.address_text or '—'}</td>
          <td style="color:var(--text-muted);font-size:.75rem;">{reporter}</td>
          <td style="color:var(--text-muted);font-size:.75rem;">{r.reporter_phone or '—'}</td>
          <td style="color:var(--text-muted);font-size:.75rem;">{r.created_at.strftime('%d %b %Y %H:%M')}</td>
        </tr>"""

    sp_opts  = '<option value="">All Species</option>' + "".join(f'<option value="{s}" {"selected" if species==s else ""}>{s.title()}</option>' for s in SPECIES_CLASSES)
    sev_opts = '<option value="">All Severity</option>' + "".join(f'<option value="{s}" {"selected" if severity==s else ""}>{s.title()}</option>' for s in SEVERITY_CLASSES)
    st_opts  = '<option value="">All Status</option>' + "".join(f'<option value="{s}" {"selected" if status==s else ""}>{s.replace("_"," ").title()}</option>' for s in ["pending","dispatched","in_progress","rescued","closed"])

    # Stats
    total_r   = Report.query.count()
    rescued_r = Report.query.filter_by(status="rescued").count()
    sos_r     = Report.query.filter_by(is_sos=True).count()
    pending_r = Report.query.filter_by(status="pending").count()
    confirmed_r = Report.query.filter(Report.user_species.isnot(None)).count()

    content = f"""
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
  <div>
    <h4 style="font-weight:800;color:#fff;margin:0;"><i class="fa fa-database me-2" style="color:#00e676;"></i>All Reports — Records Database</h4>
    <p style="color:var(--text-muted);font-size:.83rem;margin:0;">{total_count} reports found · Showing up to 50 per page</p>
  </div>
  <a href="/api/export" class="btn btn-ghost btn-sm"><i class="fa fa-download me-1"></i>Export CSV</a>
</div>
<div class="row g-3 mb-4">
  <div class="col-6 col-md-3"><div class="stat-card blue"><div class="sc-num" data-count="{total_r}">{total_r}</div><div class="sc-lbl">Total Reports</div></div></div>
  <div class="col-6 col-md-3"><div class="stat-card green"><div class="sc-num" data-count="{rescued_r}">{rescued_r}</div><div class="sc-lbl">Rescued</div></div></div>
  <div class="col-6 col-md-3"><div class="stat-card orange"><div class="sc-num" data-count="{pending_r}">{pending_r}</div><div class="sc-lbl">Pending</div></div></div>
  <div class="col-6 col-md-3"><div class="stat-card red"><div class="sc-num" data-count="{sos_r}">{sos_r}</div><div class="sc-lbl">SOS Cases</div></div></div>
</div>
<div class="glass-card p-3 mb-3">
  <form method="GET" action="/records" class="row g-2 align-items-end">
    <div class="col-6 col-md-2"><label class="form-label">Species</label><select name="species" class="form-select form-select-sm">{sp_opts}</select></div>
    <div class="col-6 col-md-2"><label class="form-label">Severity</label><select name="severity" class="form-select form-select-sm">{sev_opts}</select></div>
    <div class="col-6 col-md-2"><label class="form-label">Status</label><select name="status" class="form-select form-select-sm">{st_opts}</select></div>
    <div class="col-6 col-md-3"><label class="form-label">City</label><input type="text" name="city" value="{city}" class="form-control form-control-sm" placeholder="Filter by city..."></div>
    <div class="col-12 col-md-3 d-flex gap-2">
      <button type="submit" class="btn btn-glow btn-sm flex-fill"><i class="fa fa-filter me-1"></i>Filter</button>
      <a href="/records" class="btn btn-ghost btn-sm">Clear</a>
    </div>
  </form>
</div>
<div class="records-wrap">
  <table>
    <thead><tr><th>#</th><th>Animal</th><th>Severity</th><th>Status</th><th>Location</th><th>Reporter</th><th>Phone</th><th>Date & Time</th></tr></thead>
    <tbody>{rows if rows else '<tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--text-muted);">No reports match your filters.</td></tr>'}</tbody>
  </table>
</div>
<p style="font-size:.75rem;color:var(--text-muted);margin-top:.8rem;text-align:center;">Click any row to view full report details · ✓ = species confirmed by reporter · (AI) = AI prediction only</p>"""
    return render_page(content)
...
def export_csv():
    ...
    lines = ["id,species,user_confirmed_species,severity,user_confirmed_severity,status,city,address,latitude,longitude,reporter_name,reporter_phone,description,urgent_votes,is_sos,dispatched_at,rescued_at,created_at"]
    for r in reports:
        lines.append(",".join(str(x or "").replace(",","") for x in [
            r.id, r.predicted_species, r.user_species or "", r.predicted_severity, r.user_severity or "",
            r.status, r.city or "", r.address_text or "", r.latitude, r.longitude,
            r.reporter_name or "", r.reporter_phone or "", (r.description or "").replace("\n"," "),
            r.urgent_votes or 0, int(r.is_sos or 0),
            r.dispatched_at.strftime("%Y-%m-%d %H:%M") if r.dispatched_at else "",
            r.rescued_at.strftime("%Y-%m-%d %H:%M") if r.rescued_at else "",
            r.created_at.strftime("%Y-%m-%d %H:%M")
        ]))
    return Response("\n".join(lines), mimetype="text/csv",
                    headers={"Content-Disposition": "attachment;filename=pawalert_records.csv"})
...
if __name__ == "__main__":
    with app.app_context():
        # Drop and recreate if schema changed (new columns added in v3)
        try:
            from sqlalchemy import inspect, text
            inspector = inspect(db.engine)
            cols = [c['name'] for c in inspector.get_columns('reports')]
            if 'user_species' not in cols or 'reporter_name' not in cols:
                print("[PawAlert] Schema updated — rebuilding database...")
                db.drop_all()
                db.create_all()
                print("[PawAlert] ✓ Database rebuilt with new schema")
            else:
                db.create_all()
        except:
            db.drop_all()
        ...
        seed_ngos()
        print("\n" + "="*62)
        print("  🐾  PawAlert v3.0 is running!")
        print("  Home       : http://localhost:5000")
        print("  Live Map   : http://localhost:5000/map")
        print("  Records DB : http://localhost:5000/records")
        ...
        print("  Export CSV : http://localhost:5000/api/export")
        print("  NEW: Species selector (user picks animal type)")
        print("  NEW: Severity selector (user picks how bad it is)")
        print("  NEW: Reporter name + phone saved with every report")
        print("  NEW: Records page with filter + CSV export")
        print("="*62 + "\n")
